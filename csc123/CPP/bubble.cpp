...
The bubblesort procedure can sort an array of numbers, but can it sort any type that defines the < operator?
The `roster` type contains a pointer to a heap-allocated array, and < compares the sizes of these arrays.
Can bubblesort sort an array of rosters?

1.  (most important): Identify the cause of the error.
    I'm not asking you what the program "should" look like, but to understand exactly what caused the error!
    What kind of memory error (memory leak, dangling pointer, etc) is it, and which line(s) caused it.
    Be as specific as possible (don't just say it's using raw pointers).
...
3.  Did part 2 fix the problem?
    If not, EXPLAIN WHY.
    Then fix any new problems by modifying bubblesort.
    Bubblesort must be able to sort an array of rosters as well as any type that defines the < operator.
...
/*
1.  The cause of the errorwas in the bubblesort function.
    When this line of code was run: "T temp = A[k];", all it did was create another pointer pointing to the same location in memory.
    Therefore, when this line of code ran: "A[k] = A[k+1];", a memory leak was created and the original value of A[k] was lost.
3.  Not fully, it didn't fix the problems listed in the answer to question 1.
*/