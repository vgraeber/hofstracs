...
2. Correct the error.  Once you understand what's the cause of the error, 
   only few more lines of code is required to correct it. Your code must
   stay efficient (no cloning!). And you may NOT change main.
...
WARNING: this assignment has nothing to do with stock investments, it's
  about correct memory usage. Do not give me some AI-generated garbage
  where the stocks and investors interact in a different way.  
  You must understand what's causing the error here and write code that 
  addresses ONLY that error.  Changing main will result in a grade of -10000.
*/
...
/*
The Observer Design Pattern is an object oriented design that's proven
to be effective in industry.  It requires all the elements of
object-oriented programming including inheritance, dynamic memory
allocation and dynamic typing.  There are two hierarchies of objects:
observers and observed (subjects).  Each observed object contains a
set of pointers to observers.  The observed objects must implement an
"attach" and a "detach" function to add/remove observers.  It must
also implement a "notify" function, which notifies all attached
observers of some change in the state of the observed object.  Each
observer can observe (attached to) multiple subjects.  When an
observer is notified, it runs a function "update".  There are usually
different types of observers, each with its own update function, and
will react differently.

In our example, the observed objects are stocks and the observers are
investors.  A struct (class) `stock` is defined.  Each stock has a
symbol and a price history represented by a stack (vector) of floating
point values: the top of the stack contains the most recent price.
When the price changes, a new price value is pushed on the stack and
*all investors are notified*.  For investors, there is a superclass
`investor` subclasses bullinvestor, bearinvestor and panickyinvestor,
each with their own "update" function that react to notifications from
stocks.  Each update function represents a different investment
strategy.
...
When we iterate over the portfolio map in an investor, we are iterating
over items that are nested pairs of the form (string,(shared_ptr,int)), 
so, for example, item.second.first refers to the shared pointer to a stock.
*/
