...
Afterwards I receive a Windows message saying that the programm stopped because I have not enough memory.
Thing is I am using dynamic memory for the program and I am afraid I have done something wrong with it...
...
"
Welcome to the C++ program for Molecular Dynamics simulation.
Introduce how many particles you want to work with: 10000
...
Instead of double**, use the following types for dynamically allocated 2D array:
...
Remove all uses of raw pointers and replace them with smart pointers and references.
You must be careful to use a reference (matrix& or matrix&&) where appropriate.
The resulting program cannot be less efficient than the original program was intended to be.

This is the length of a "real world" program, though it's actually not that long.
Most of what you need to replace is relatively straightforward.
But be careful.
For example, consider the void populate(double** m) function.
Do not just change this to void populate(double2d m): because double2d is a unique_ptr, which means there must be a transfer of ownership (move) to the local m when the function is called.
This means that whatever double2d you passed to it would be destroyed after the call!
Instead, change the signature of this function to void populate(double2d& m);
Mixing smart pointers with references is OK.

Also uncomment the call to operator_testing() inside main when you've fixed the program.
*/
...
typedef std::unique_ptr<double[]> double1d;
typedef std::unique_ptr<double1d[]> double2d;
...
struct matrix {
	...
	public:
		...
		double2d alloc(int rows, int cols);
		void printm();
		double2d& getmatrix();
		void trans();
		int nrows();
...
// allocate memory
matrix::matrix() {
	the_matrix = move(alloc(0, 0));
};
// constructor of the matrix as object
matrix::matrix(int rows, int cols) {
	the_matrix = move(alloc(rows, cols));
}
...
// get matrix elements as double pointer address
double2d& matrix::getmatrix() {
	return the_matrix;
}

// transpose matrix
void matrix::trans() {
	double2d transpose = make_unique<double1d[]>(y);
	for(int i = 0; i < y; i++) {
		transpose[i] = make_unique<double[]>(x);
	}
	for(int i = 0; i < y; i++) {
		for(int j = 0; j < x; j++) {
...
void matrix::populate(double2d& pp) {}
...
void matrix::multiply(matrix& m) {
	double2d producto = make_unique<double1d[]>(x);

	for(int i = 0; i < x; i++) {
		producto[i] = make_unique<double[]>(m.ncolumns());
	}
...
// main
double1d v_rand_MB(int npart, double a) {// arguments are # of particles and parameter "a", related to temperature of the gas
	...
	vmax = 5.0 * a;
	interval = abs(vmax);
	myp = make_unique<double[]>(npart);// The pointer points towards an npart dimensional array of type double
...
void initiate_velocities(matrix& m, double1d& myp) {
...
void initiate_positions(matrix& m, int Npart) {
	int count, ylim, zlim;
	double density, epsilon, dx, x, y, z;// density and minimum radius associated to each particle, and epsilon is the value in order to avoid one particle lying on one of the faces of the cube;
	epsilon = 0.1;
	// The box lives in the interval -1/2 + 1/2 for each coordinate.
	density = 1.0 / (Npart);
	// We need to generate NxNxN positions (a cube).
	// We start by creating the points in the z =+ epsilon level, and we rise them till z = 1 - epsilon.
...
int main() {
  operator_testing();// UNCOMMENT AFTER FIXING PROGRAM
	...
	double tf, dt, a, gamma, beta;// Final time, differential of time, temp.parameter MB distribution.
	// The parameters gamma and r0 belong to the Buckinham potential, and must be chosen carefully.
	gamma = 0.2;
	beta = 0.02;
	
	double1d vpoint;// Pointer to velocities distribution
	...
	// Lets use a Maxwell-Boltzmann distribution for the velocities, whereas the position of particles should be occupying vertices of a cube in space.
	// Matrices of initial positions and positions at time t, and for velocities., and the matrix of relative distances between particles, relevant for the forces acting over each individual particle
...
/*
1.	The "clean()" function is identical to the destructor, so using it on matrices and not redefining them is a bug, since it results in a double delete.
		The "delete[] vpoint;//Free memory block" is also a bug, as vpoint is not assigned to anything else and causes another double free.
*/